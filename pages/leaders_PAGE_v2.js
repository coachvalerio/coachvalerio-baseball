...
// ── #13 ISR: prefetch current season server-side, revalidate every 5 min ──
export async function getStaticProps() {
  try {
    const base = process.env.NEXT_PUBLIC_BASE_URL || 'https://www.coachvalerio.com';
    const r = await fetch(`${base}/api/leaders?season=${CUR_YEAR}`, { signal: AbortSignal.timeout(9000) });
    const initialData = r.ok ? await r.json() : null;
...
export default function LeadersPage({ initialData }) {
  ...
  const load = useCallback((yr) => {
    // Current season: use ISR-prefetched data instantly (no client fetch)
    if (yr === CUR_YEAR && initialData) {
      setData(initialData);
      setLoading(false);
      return;
    }
